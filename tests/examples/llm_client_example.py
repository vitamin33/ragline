...
from rag.llm_client import ChatMessage, LLMClient, LLMConfig
...
async def test_llm_client():
    ...
    config = LLMConfig(model="gpt-4o-mini", max_tokens=100, temperature=0.7)
    ...
    try:
        stream_messages = [ChatMessage(role="user", content="Count to 5 slowly")]
    ...
    try:
        response = await client.chat_completion(
            tool_messages, tools=tools, stream=False
        )
