...
async def test_tool_rag_integration():
    ...
    try:
                ...
                if result.success:
                    ...
                    if data.get("items"):
                        ...
                        if "rag_relevance" in first_item:
                            ...
                            print(f"      ⭐ Relevance: {relevance.get('similarity_score', 0):.3f} - {relevance.get('retrieval_reason', 'N/A')}")
        ...
        promo_scenarios = [
            {"name": "Valid Promo", "args": {"promo_code": "SAVE20", "order_total": 30.00}},
            {"name": "Invalid Promo", "args": {"promo_code": "BADCODE123", "order_total": 25.00}}
        ]
