...
async def test_connection_manager():
    ...
    try:
        from services.worker.tasks.notifications import ConnectionManager, ConnectedClient
        ...
        stats_valid = all(key in stats for key in [
            'total_connections', 'healthy_connections', 'connections_by_type'
        ])
...
async def test_stream_subscription():
    ...
    try:
        ...
        print(f"   ✅ Stream notifier created")
        ...
        expected_streams = [StreamTopic.ORDERS, StreamTopic.USERS, StreamTopic.NOTIFICATIONS]
...
async def test_notification_fanout():
    ...
    try:
        from services.worker.tasks.notifications import StreamNotifier, ConnectedClient
        from services.worker.config import WorkerConfig
...
async def test_celery_notification_tasks():
    ...
    try:
        from services.worker.tasks.notifications import (
            add_client_connection, remove_client_connection, 
            get_notifier_stats, send_test_notification
        )
...
async def test_stream_to_notification_flow():
    ...
    try:
        ...
        test_events = [
            {
                "status": OrderStatus.CREATED,
                "reason": "Flow test - order created"
            },
            {
                "status": OrderStatus.CONFIRMED,
                "reason": "Flow test - order confirmed"
            }
        ]
        ...
        if published_events > 0:
            ...
            try:
                await notifier._process_stream(
                    ...
                    notifier.stream_configs[StreamTopic.ORDERS]
                )
                
                processing_works = True
                print(f"   ✅ Stream processing simulation: Success")
...
async def test_backpressure_handling():
    ...
    try:
        ...
        print(f"      📊 Connections added: {added_connections} (limit: {notifier.connection_manager.max_connections_per_user})")
