...
async def demo_notification_system():
    ...
    try:
        from services.worker.tasks.notifications import (
            StreamNotifier, ConnectionManager, ConnectedClient
        )
        from services.worker.config import WorkerConfig
        from packages.orchestrator.stream_producer import get_stream_producer, StreamEvent, EventMetadata
    ...
    try:
        ...
        print(f"   ✅ Stream notifier initialized")
    ...
    try:
        ...
        for client_info in clients:
            if notifier.connection_manager.add_connection(client_info["client"]):
                connected_clients += 1
                print(f"   ✅ {client_info['name']}: Connected ({client_info['client'].connection_type})")
    ...
    try:
        ...
        for i, event_info in enumerate(order_events):
            ...
            order_event = EventFactory.create_enriched_order_event(
                tenant_id=uuid.UUID("12345678-1234-5678-9012-123456789abc"),  # Fixed for demo
                order_id=order_id,
                status=event_info["status"],
                user_id=uuid.UUID("87654321-4321-8765-4321-987654321abc"),   # Fixed for demo
    ...
    try:
        ...
        if stream_length > 0:
            ...
            try:
                recent_messages = await client.client.xrevrange("ragline:stream:orders", count=3)
                
                print(f"   📨 Recent messages in stream:")
        ...
        for event_step, message_id in published_events:
            try:
                ...
                recipients = notifier.connection_manager.get_connections_for_event(event_data)
