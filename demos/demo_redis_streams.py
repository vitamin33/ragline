...
async def demo_redis_streams():
    ...
    try:
        ...
        for topic, config in producer.stream_configs.items():
            print(
                f"      📍 {topic.value}: max_len={config.max_len}, group={config.consumer_group}"
            )
    ...
    try:
        ...
        if "redis_client_metrics" in metrics:
            ...
            print(
                f"   📈 Redis success rate: {redis_metrics.get('success_rate', 0):.1f}%"
            )
    ...
    print(
        "📋 Event flows: Database → Outbox → Stream Producer → Redis Streams → Consumers"
    )
