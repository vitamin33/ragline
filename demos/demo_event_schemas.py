...
async def demo_event_schemas():
    ...
    print("📋 Business Scenario: Order Lifecycle")
    ...
    print(
        f"   ✅ Confirmed event: {confirmed_event.status.value} with correlation {correlation_id}"
    )
    ...
    for event_name, event in lifecycle_events:
        try:
            ...
            event_dict = (
                event.to_dict() if hasattr(event, "to_dict") else event.model_dump()
            )
            ...
            print(
                f"   ✅ {event_name}: {'Valid' if is_valid else 'Invalid'} against schema"
            )
    ...
    for event_name, event in lifecycle_events:
        try:
            ...
            # Verify integrity
            integrity_check = (
                reconstructed.order_id == event.order_id
                and reconstructed.status == event.status
                and reconstructed.event == event.event
            )
            print(
                f"   ✅ {event_name}: Integrity check {'passed' if integrity_check else 'failed'}"
            )
        ...
        if stream_length > 0:
            ...
            try:
                ...
                for msg_id, fields in recent_messages:
                    ...
                    print(
                        f"      {msg_id.decode()}: {event_type} for order {order_ref}"
                    )
    ...
    try:
        ...
        print("   ✅ Contract schema loaded:")
        print(f"      Schema version: {schema.get('$schema', 'unknown')}")
        print(f"      Required fields: {len(schema.get('required', []))}")
        print(
            f"      Event types: {schema.get('properties', {}).get('event', {}).get('enum', [])}"
        )
        print(
            f"      Status values: {schema.get('properties', {}).get('status', {}).get('enum', [])}"
        )
