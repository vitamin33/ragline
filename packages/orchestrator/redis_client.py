...
class RedisStreamClient:
    ...
    async def create_consumer_group(self, stream_config: StreamConfig, mkstream: bool = True):
        ...
        return await self._execute_with_retry(
            f"create_consumer_group_{stream_config.name}",
            _create_group
        )
    
    async def add_to_stream(self, stream_name: str, fields: Dict[str, str], 
                           max_len: Optional[int] = None, message_id: str = '*') -> str:
    ...
    async def read_from_stream(self, stream_config: StreamConfig, 
                              from_id: str = '>') -> List[StreamMessage]:
        ...
        return await self._execute_with_retry(
            f"read_from_stream_{stream_config.name}",
            _read_messages
        )
    ...
    async def acknowledge_message(self, stream_name: str, consumer_group: str, message_id: str):
        ...
        return await self._execute_with_retry(
            f"ack_message_{stream_name}_{message_id}",
            _ack_message
        )
    ...
    async def get_stream_info(self, stream_name: str) -> Dict[str, Any]:
        ...
        try:
            return await self._execute_with_retry(
                f"get_stream_info_{stream_name}",
                _get_info
            )
    ...
    async def get_consumer_group_info(self, stream_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            return await self._execute_with_retry(
                f"get_consumer_group_info_{stream_name}",
                _get_group_info
            )
        except ResponseError:
    ...
    async def claim_pending_messages(self, stream_config: StreamConfig, 
                                   min_idle_time: Optional[int] = None) -> List[StreamMessage]:
        ...
        return await self._execute_with_retry(
            f"claim_pending_{stream_config.name}",
            _claim_messages
        )
    ...
    async def delete_consumer(self, stream_name: str, consumer_group: str, consumer_name: str):
        ...
        return await self._execute_with_retry(
            f"delete_consumer_{stream_name}_{consumer_name}",
            _delete_consumer
        )
