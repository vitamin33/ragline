...
class OrderV1Event(BaseModel):
    ...
    def validate_event_consistency(self):
        ...
        if self.event == OrderEvent.ORDER_STATUS:
            if self.status not in [OrderStatus.CREATED, OrderStatus.CONFIRMED, OrderStatus.FAILED]:
...
class EventSerializer:
    ...
    @staticmethod
    def serialize_to_stream_fields(event: Union[OrderV1Event, BaseEvent]) -> Dict[str, str]:
        """
    ...
    @staticmethod
    def deserialize_from_stream_fields(fields: Dict[str, str], event_type: str = "base") -> Union[OrderV1Event, BaseEvent]:
        """
...
def validate_event_structure(event_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Additional warnings
        required_fields = ['event_id', 'timestamp'] if validation_result['event_type'] == 'base' else ['event', 'version', 'tenant_id', 'order_id', 'status', 'ts']
