...
async def test_complete_agent_b_functionality():
    ...
    try:
        ...
        schema_working = (
            reconstructed.order_id == order_event.order_id
            and len(stream_fields) >= 6
            and all(isinstance(v, str) for v in stream_fields.values())
        )
    ...
    try:
        ...
        streams_working = bool(message_id) and stream_info.get("length", 0) > 0
