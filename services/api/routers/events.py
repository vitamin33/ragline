    ...
    async def add_connection(self, connection: WebSocketConnection) -> bool:
        ...
        async with self._lock:
            try:
                ...
                logger.error("Failed to add WebSocket connection", client_id=connection.client_id, error=str(e))
                return False
...
async def stream_notifications(
    ...
    async def notification_generator():
        ...
        try:
                        ...
                        for stream, msgs in messages:
                            for msg_id, fields in msgs:
                                ...
                                if (event_tenant_id and str(event_tenant_id) == str(tenant_id)):
                                    ...
                                    if (event_type in ["order_created", "order_completed", "order_failed", 
            ...
            yield {
                "event": "error", 
                "data": json.dumps({"stream": "notifications", "message": "Stream failed", "error": str(e)})
            }
        finally:
            # Redis client cleanup handled by connection pool
            pass
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.error("WebSocket message processing error", client_id=client_id, error=str(e))
                await connection.send_message({
                    "type": "error",
...
async def handle_websocket_message(connection: WebSocketConnection, message: str):
    ...
    try:
        ...
        logger.error("WebSocket message handling error", client_id=connection.client_id, error=str(e))
        await connection.send_message({
            "type": "error",
...
async def websocket_orders(websocket: WebSocket):
    ...
    try:
        ...
        connection.subscriptions = {"order_created", "order_updated", "order_completed", "order_failed"}
        ...
        logger.info("WebSocket orders connected", client_id=client_id, tenant_id=token_data.tenant_id)
